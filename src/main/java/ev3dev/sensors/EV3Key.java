 ...
 *
 * The easiest way to get an instance of this class is to use one of the static constants of the {@link Button} class.
 *
...
public class EV3Key implements Key {
    ...
    static {
        Arrays.stream(KeyType.values()).forEach((type) -> keyListeners.put(
                type, new CopyOnWriteArraySet<>()));  // the CopyOfWriteArraySet will spare us a lot of hussle with thread safety
    }
        ...
        keyEventReader = new Thread(() -> {
            ...
            try (final DataInputStream in = new DataInputStream(new FileInputStream(SYSTEM_EVENT_PATH))) {
                while (true) {
                    ...
                    if (keyId != 0) {  // for some reason, every event is followed with another one where everything except the timestamp is zero
    ...
    enum KeyType {
        ...
        public static KeyType of(final int id) {
            return Optional.ofNullable(LOOKUP.get((byte)id))
                    .orElseThrow(() -> new IllegalArgumentException(String.format("No such keyType (%d), please use one of the EV3Key.BUTTON_* constants", id)));
        }
     ...
     *
     * This method returns immediately if this key is pressed already.
     */
    @Override
     ...
     *
     * Unlike {@link #waitForPress()}, this method will always block because this key cannot be already
    ...
    public void waitForPressAndRelease() {
        this.waitForPress();   // this will either return immediately of this key is already pressed or wait until it gets pressed
        synchronized (keyEventReader) {
            while (this.isDown()) {
                try {
                    keyEventReader.wait();
    ...
    // TODO: ... or not - Threads consume resources and notifying in a separate Thread also means the events may no longer come in correct order
